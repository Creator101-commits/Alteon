...
const getFirebaseData = () => import("./firebase-data");
...
export const signInWithGoogle = async (enableSync: boolean = true) => {
  try {
    ...
    if (token) {
      try {
        const { setUserRecord } = await getFirebaseData();
        await setUserRecord(user.uid, {
          email: user.email,
...
export const getUserData = async (uid: string) => {
  try {
    const { getUserRecord } = await getFirebaseData();
...
export const updateUserData = async (uid: string, data: any) => {
  try {
    const { updateUserRecord } = await getFirebaseData();
    await updateUserRecord(uid, { ...data, updatedAt: new Date() });
...
export const signInWithEmail = async (email: string, password: string) => {
  try {
    ...
    try {
      const { updateUserRecord } = await getFirebaseData();
      await updateUserRecord(user.uid, { lastSignIn: new Date() });
    } catch (firestoreError) {
